...
Nesta etapa os grupos deverão implementar a **geração de código de três endereços (TAC)** para as construções mais simples da linguagem LARA: expressões aritméticas e relacionais, declarações de variáveis (com cálculo de *offset* na tabela de símbolos), e atribuições simples.
...
O TAC (*Three-Address Code*) da linguagem LARA usa o formato de **quádrupla**:
